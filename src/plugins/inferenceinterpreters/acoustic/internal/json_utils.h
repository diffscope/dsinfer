...
namespace dsinfer {
    ...
    inline bool get_input(const std::string &className, const JsonValue &json, const std::string &key, Error *error,
                     T &outValue) {
        const JsonValue val = json[key];
        if (val.isUndefined()) {
            if (error) {
                *error = Error(Error::InvalidFormat,
                "Invalid format of " + className + ": missing key \"" + key + "\" of " + detail::type_to_string<T>() +
                "value");
            }
            return false;
        }
        if (!detail::is_json_type<T>(val)) {
            if (error) {
                *error = Error(Error::InvalidFormat,
                               "Invalid format of " + className + ": key \"" + key +
                               "\" is not " + detail::type_to_string<T>());
            }
            return false;
        }
        outValue = detail::to_json_type<T>(val);
        return true;
    }

    template<typename T>
    inline bool get_input(const std::string &className, const JsonObject &json, const std::string &key, Error *error,
                 T &outValue) {
